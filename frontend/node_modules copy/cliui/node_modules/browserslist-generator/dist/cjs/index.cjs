...
function normalizeBrowserslist(browserslist) {
    ...
    for (const browser of ["and_ff", "and_chr", "op_mini"]) {
        const versions = getSortedBrowserVersions(browser);
...
function browsersWithSupportForFeatures(...args) {
    const [firstArg] = args;
    const { features } = firstArg != null && typeof firstArg === "object" ? firstArg : { features: args };
 ...
 */
function browserslistSupportsEcmaVersion(browserslist, version, options) {
    const features = getFeaturesForEcmaVersion(version, options);
    if (features.length === 0)
        return true;
    return browserslistSupportsFeatures(browserslist, ...features);
}
function getFeaturesForEcmaVersion(version, options) {
    const features = getFeaturesForEcmaVersionInner(version);
    const filteredFirstPass = Boolean(options === null || options === void 0 ? void 0 : options.syntaxOnly) ? features.filter(feature => !feature.startsWith("javascript.builtins.")) : features;
    return typeof (options === null || options === void 0 ? void 0 : options.featureFilter) === "function" ? filteredFirstPass.filter(options.featureFilter) : filteredFirstPass;
}
function getFeaturesForEcmaVersionInner(version) {
    switch (version) {
        case "es3":
 ...
 */
function getAppropriateEcmaVersionForBrowserslist(browserslist, options) {
    if (browserslistSupportsEcmaVersion(browserslist, "es2024", options))
        return "es2024";
    if (browserslistSupportsEcmaVersion(browserslist, "es2023", options))
        return "es2023";
    if (browserslistSupportsEcmaVersion(browserslist, "es2022", options))
        return "es2022";
    if (browserslistSupportsEcmaVersion(browserslist, "es2021", options))
        return "es2021";
    if (browserslistSupportsEcmaVersion(browserslist, "es2020", options))
        return "es2020";
    if (browserslistSupportsEcmaVersion(browserslist, "es2019", options))
        return "es2019";
    if (browserslistSupportsEcmaVersion(browserslist, "es2018", options))
        return "es2018";
    else if (browserslistSupportsEcmaVersion(browserslist, "es2017", options))
        return "es2017";
    else if (browserslistSupportsEcmaVersion(browserslist, "es2016", options))
        return "es2016";
    else if (browserslistSupportsEcmaVersion(browserslist, "es2015", options))
        return "es2015";
    else if (browserslistSupportsEcmaVersion(browserslist, "es5", options))
        return "es5";
    else
        return "es3";
}
 ...
 */
function browsersWithSupportForEcmaVersion(version, options) {
    switch (version) {
        case "es3":
...
function browsersWithoutSupportForFeatures(...args) {
    const [firstArg] = args;
    const { features } = firstArg != null && typeof firstArg === "object" ? firstArg : { features: args };
...
function shouldIgnoreBrowser(browser, version) {
    ...
    browser === "and_qq" ||
        ...
        browser === "kaios" ||
        (browser === "android" && semver.gt(coerceToString(browser, version), coerceToString(browser, "4.4.4"))) ||
...
exports.getFeaturesForEcmaVersion = getFeaturesForEcmaVersion;
